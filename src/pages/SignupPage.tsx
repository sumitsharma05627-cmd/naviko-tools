...
export const SignupPage: React.FC<SignupPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage('A network error occurred during signup. Please try again.');
    } finally {
      ...
      <div className="w-full max-w-md space-y-8 bg-white dark:bg-slate-900 p-8 sm:p-10 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none transition-all">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Join NAVIKO for free to save your tools, track analytics, and manage subscriptions.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
              Password <span className="normal-case font-normal text-slate-400">(min. 8 characters)</span>
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <label htmlFor="terms" className="text-xs text-slate-600 dark:text-slate-400 select-none cursor-pointer">
              I agree to the NAVIKO Terms of Service and Privacy Policy.
            </label>
          </div>
